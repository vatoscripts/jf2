  ...
  stages {
    ...
    stage('Build') {
      ...
      steps {
          ...
          // Build new image
          sh "docker build -t kiyange26773/jf2:${env.GIT_COMMIT} ."
          // Publish new image
          sh "docker login --username $DOCKERHUB_CREDS_USR --password $DOCKERHUB_CREDS_PSW && docker push kiyange26773/jf2:${env.GIT_COMMIT}"
    ...
    stage('Deploy to Prod') {
      steps {
        input message:'Really Deploy?'
          ...
          dir("argocd-demo-deploy") {
            sh "cd ./prod && ls && kustomize edit set image kiyange26773/jf2:${env.GIT_COMMIT}"
